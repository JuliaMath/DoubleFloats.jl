...
function randpm(::Type{DoubleFloat{T}}) where {T}
    r = rand(DoubleFloat{T})
    r = rand(Bool) ? r : -r
    return r
end

function randpm(::Type{Complex{DoubleFloat{T}}}) where {T}
    re = randpm(DoubleFloat{T})
    im = randpm(DoubleFloat{T})
    return Complex{DoubleFloat{T}}(re, im)
end

function randpm(::Type{DoubleFloat{T}}, n::Int) where {T}
    rs = rand(DoubleFloat{T}, n)
    sgns = ones(n) .- (2 .* rand(Bool, n))
    return rs .* sgns
end

function randpm(::Type{Complex{DoubleFloat{T}}}, n::Int) where {T}
