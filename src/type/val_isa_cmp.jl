...
zero(::Type{FastDouble{T,E}}) where {T<:Real,E<:Emphasis} =
    FastDouble(E, zero(T), zero(T))
iszero(x::FastDouble{T,E}) where {T<:Real,E<:Emphasis} =
    iszero(HI(x)) && iszero(LO(x))

one(::Type{FastDouble{T,E}}) where {T<:Real,E<:Emphasis} =
   FastDouble(E, one(T), zero(T))
isone(x::FastDouble{T,E}) where {T<:Real,E<:Emphasis} =
    isone(HI(x)) && iszero(LO(x))

nan(::Type{FastDouble{T,E}}) where {T<:Real,E<:Emphasis} =
   FastDouble(E, T(NaN), zero(T))
isnan(x::FastDouble{T,E}) where {T<:Real,E<:Emphasis} =
    isnan(HI(x)) && iszero(LO(x))

inf(::Type{FastDouble{T,E}}) where {T<:Real,E<:Emphasis} =
   FastDouble(E, T(Inf), zero(T))
isinf(x::FastDouble{T,E}) where {T<:Real,E<:Emphasis} =
    isinf(HI(x)) && iszero(LO(x))

isfinite(x::FastDouble{T,E}) where {T<:Real,E<:Emphasis} =
    isfinite(HI(x)) && isfinite(LO(x))
isinteger(x::FastDouble{T,E}) where {T<:Real,E<:Emphasis} =
    isinteger(LO(x)) && isinteger(HI(x))
isodd(x::FastDouble{T,E}) where {T<:Real,E<:Emphasis} =
    iszero(LO(x)) ? isinteger(HI(x)) && isodd(HI(x)) : isinteger(LO(x)) && isodd(LO(x))
iseven(x::FastDouble{T,E}) where {T<:Real,E<:Emphasis} =
    iszero(LO(x)) ? isinteger(HI(x)) && iseven(HI(x)) : isinteger(LO(x)) && iseven(LO(x))

issubnormal(x::FastDouble{T,E}) where {T<:Real,E<:Emphasis} =
    issubnormal(HI(x)) || issubnormal(LO(x))



@inline (==)(x::FastDouble{T,E}, y::FastDouble{T,E}) where {T<:Real,E<:Emphasis} = (LO(x) === LO(y)) && (HI(x) === HI(y))
@inline (!=)(x::FastDouble{T,E}, y::FastDouble{T,E}) where {T<:Real,E<:Emphasis} = (LO(x) !== LO(y)) || (HI(x) !== HI(y))
@inline (<)(x::FastDouble{T,E}, y::FastDouble{T,E}) where {T<:Real,E<:Emphasis} = (HI(x) < HI(y)) || (HI(x) === HI(y) && LO(x) < LO(y))
@inline (>)(x::FastDouble{T,E}, y::FastDouble{T,E}) where {T<:Real,E<:Emphasis} = (HI(x) > HI(y)) || (HI(x) === HI(y) && LO(x) > LO(y))
@inline (<=)(x::FastDouble{T,E}, y::FastDouble{T,E}) where {T<:Real,E<:Emphasis} = (HI(x) < HI(y)) || (HI(x) === HI(y) && LO(x) <= LO(y))
@inline (>=)(x::FastDouble{T,E}, y::FastDouble{T,E}) where {T<:Real,E<:Emphasis} = (HI(x) > HI(y)) || (HI(x) === HI(y) && LO(x) >= LO(y))

@inline isequal(x::FastDouble{T,E}, y::FastDouble{T,E}) where {T<:Real,E<:Emphasis} = (x == y)
@inline isless(x::FastDouble{T,E}, y::FastDouble{T,E}) where {T<:Real,E<:Emphasis} = (x < y)


@inline (==)(x::FastDouble{T,E}, y::T) where {T<:Real, E<:Emphasis} = iszero(LO(x)) && (HI(x) === y)
@inline (==)(x::T, y::FastDouble{T,E}) where {T<:Real, E<:Emphasis} = iszero(LO(y)) && (HI(y) === x)
@inline (!=)(x::FastDouble{T,E}, y::T) where {T<:Real, E<:Emphasis} = !iszero(LO(x)) || (HI(x) !== y)
@inline (!=)(x::T, y::FastDouble{T,E}) where {T<:Real, E<:Emphasis} = !iszero(LO(y)) || (HI(y) !== x)
@inline (<)(x::FastDouble{T,E}, y::T) where {T<:Real, E<:Emphasis} = (HI(x) < y) || ((HI(x) == y) && signbit(LO(x)))
@inline (<)(x::T, y::FastDouble{T,E}) where {T<:Real, E<:Emphasis} = (x < HI(y)) || ((x == HI(y)) && !signbit(LO(y)))
@inline (>)(x::FastDouble{T,E}, y::T) where {T<:Real, E<:Emphasis} = (HI(x) > y) || ((HI(x) == y) && (LO(x) < zero(T)))
@inline (>)(x::T, y::FastDouble{T,E}) where {T<:Real, E<:Emphasis} = (x > HI(y)) || ((x == HI(y)) && signbit(LO(y)))
@inline (<=)(x::FastDouble{T,E}, y::T) where {T<:Real, E<:Emphasis} = (HI(x) < y) || ((HI(x) == y) && (LO(x) <= zero(T)))
@inline (<=)(x::T, y::FastDouble{T,E}) where {T<:Real, E<:Emphasis} = (x < HI(y)) || ((x == HI(y)) && (LO(y) >= zero(T)))
@inline (>=)(x::FastDouble{T,E}, y::T) where {T<:Real, E<:Emphasis} = (HI(x) > y) || ((HI(x) == y) && (LO(x) <= zero(T)))
@inline (>=)(x::T, y::FastDouble{T,E}) where {T<:Real, E<:Emphasis} = (x > HI(y)) || ((x == HI(y)) && (LO(y) >= zero(T)))

@inline isequal(x::FastDouble{T,E}, y::T) where {T<:Real, E<:Emphasis} = (x == y)
@inline isequal(x::T, y::FastDouble{T,E}) where {T<:Real, E<:Emphasis} = (x == y)
@inline isless(x::FastDouble{T,E}, y::T) where {T<:Real, E<:Emphasis} = (x < y)
@inline isless(x::T, y::FastDouble{T,E}) where {T<:Real, E<:Emphasis} = (x < y)
